 ...
 */
export function loadCLAUDEFile(filePath) {
    try {
 ...
 */
export function findCLAUDEFiles(startPath, rootPath) {
    const files = [];
 ...
 */
export function loadCLAUDEHierarchy(startPath, rootPath) {
    const files = findCLAUDEFiles(startPath, rootPath);
    return files.map(f => loadCLAUDEFile(f)).filter((c) => c !== null);
}
/**
 * Format CLAUDE content as system prompt addition
 */
export function formatAsSystemPrompt(contents) {
    if (contents.length === 0) {
        return '';
    }
    const parts = ['# Workspace Context (from CLAUDE.md)'];
...
//# sourceMappingURL=CLAUDELoader.js.map